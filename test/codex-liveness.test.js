...
test('живая Codex-сессия остаётся, закрытая удаляется только после двух проверок', () => {
  const sessions = {
  ...
  const first = pruneClosedCodex(sessions, new Set([100]), new Map(), 1000, 10_000);
  assert.equal(first.sessions, sessions, 'первая проверка ничего не удаляет');
  assert.equal(first.missingSince.get('closed'), 1000);

  const second = pruneClosedCodex(sessions, new Set([100]), first.missingSince, 11_000, 10_000);
  assert.ok(second.sessions.live);
  assert.ok(second.sessions.claude);
...
test('повторно открытый rollout сбрасывает подозрение на закрытие', () => {
  const sessions = { s1: { agent: 'codex', processPid: 100 } };
  const missing = new Map([['s1', 1000]]);
  const result = pruneClosedCodex(sessions, new Set([100]), missing, 50_000, 10_000);
