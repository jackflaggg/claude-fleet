...
| **ждёт тебя** | `Notification`/`PermissionRequest` (разрешение) или `Stop` (закончил ход) | красный |
...
Закрытая вкладка Codex исчезает с борда сама примерно за 10–20 секунд: Fleet получает PID
процесса с hook-событием и проверяет его существование только при открытом борде, без опроса
диска и запуска фоновых утилит. Для Claude Code и аварийных случаев остаётся кнопка × в углу
карточки (появляется при наведении): без `SessionEnd` резервная уборка сработает через 6 часов.
...
Первой командой покрыты чистые ядра статусов, фокуса, ограничителей, лимита и Codex-liveness.
...
Codex-сессий проверяется системным signal 0 раз в 10 секунд: замер миллиона проверок даёт
...
- Отвечать агенту прямо из борда - нет, отвечаешь в Claude Code или Codex.
